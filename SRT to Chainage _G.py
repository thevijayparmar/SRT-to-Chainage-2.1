...
try:
    ...
    from pyproj import Geod, Proj, Transformer, CRS
    import lxml.etree
...
def display_kml_info():
    ...
    st.info(
        ...
        f"- **Total Alignment Length:** {length_km:.3f} km\n"
        f"- **Current Start Point:** {coords[0][0]:.6f}, {coords[0][1]:.6f}\n"
        f"- **Current End Point:** {coords[-1][0]:.6f}, {coords[-1][1]:.6f}"
    )
...
def run_computation():
    ...
    if PROJECTION_LIBS_AVAILABLE:
        ...
        # Fallback to a basic spherical calculation if pyproj is not available
        st.warning("Using less accurate spherical distance calculation for chainage.")
...
def calculate_linestring_length(coords: List[Tuple[float, float]]) -> float:
    """
    Calculates the total geodesic length of a linestring (in km)
    using the WGS84 ellipsoid for high accuracy, matching Google Earth.
    """
    if not PROJECTION_LIBS_AVAILABLE or len(coords) < 2:
        return 0.0

    geod = Geod(ellps='WGS84')
    total_length_meters = 0.0
    
    # Extract lats and lons into separate lists for the geod.inv function
    lats1 = [p[0] for p in coords[:-1]]
    lons1 = [p[1] for p in coords[:-1]]
    lats2 = [p[0] for p in coords[1:]]
    lons2 = [p[1] for p in coords[1:]]

    # The geod.inv method calculates the distance between point pairs
    _, _, distances = geod.inv(lons1, lats1, lons2, lats2)
    
    total_length_meters = np.sum(distances)
    
    return total_length_meters / 1000.0
...
def compute_chainage_fallback(records: List[SRTRecord], kml_coords: List[Tuple[float, float]]) -> pd.DataFrame:
    """Fallback chainage computation using simple spherical distance (less accurate)."""
    # Precompute cumulative distance along the chain using the accurate geodesic method
    cum_dist_km = [0.0]
    geod = Geod(ellps='WGS84')
    lats1 = [p[0] for p in kml_coords[:-1]]
    lons1 = [p[1] for p in kml_coords[:-1]]
    lats2 = [p[0] for p in kml_coords[1:]]
    lons2 = [p[1] for p in kml_coords[1:]]
    _, _, distances = geod.inv(lons1, lats1, lons2, lats2)
    
    for dist in distances:
        cum_dist_km.append(cum_dist_km[-1] + dist / 1000.0)

    results = []
    for rec in records:
        drone_point = (rec['lat'], rec['lon'])
        
        # Find distance from drone to each vertex on the chain
        lats_v = [v[0] for v in kml_coords]
        lons_v = [v[1] for v in kml_coords]
        
        # Vectorize distance calculation from drone point to all vertices
        _, _, distances_to_vertices_m = geod.inv(
            [drone_point[1]] * len(lons_v), 
            [drone_point[0]] * len(lats_v),
            lons_v, 
            lats_v
        )
        
        # Find the closest vertex on the chain (this is the approximation for the fallback)
        closest_vertex_idx = np.argmin(distances_to_vertices_m)
        
        chainage_km = cum_dist_km[closest_vertex_idx]
        distance_to_chain_m = distances_to_vertices_m[closest_vertex_idx]
